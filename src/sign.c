...
const char *iota_sign(const int8_t* normalizedFragment, const char* keyTrytes) {
  ...
  signedFragment = (char*) calloc(IOTA_NUMBER_OF_ROUNDS * IOTA_HASHLEN_TRYTE, sizeof(char));
  ...
  for(i = 0; i < IOTA_NUMBER_OF_ROUNDS; i++ ){
    ...
    for(j = 0; j < 13 - normalizedFragment[i]; j++) {
      hashed = (char*) iota_curl_hash_trytes(&c, signedFragment + idx, IOTA_HASHLEN_TRYTE );
...
char* digest(int8_t* normalized, const char* signFrag) {
  ...
  init_curl(&d);
  for(i = 0; i < IOTA_NUMBER_OF_ROUNDS; i++) {
    memcpy(tmp, trits + i*IOTA_HASHLEN_TRIT, IOTA_HASHLEN_TRIT*sizeof(trit_t));

    for(j = normalized[i] + 13; j > 0; j--) {
      iota_curl_hash(&c, tmp, 0, IOTA_HASHLEN_TRIT);
    }

    digested = trytes_from_trits(tmp, 0, IOTA_HASHLEN_TRIT);
...
char iota_validateSignature(const char* address, const char** signFrags, size_t len, const char* bundleHash) {
  int offset, i;
  ...
  return strncmp(digests, address, IOTA_HASHLEN_TRYTE + IOTA_CHECKSUM_SIZE); 
}
