...
const char* iota_digests(const char* trytes) {
  ...
  for(i = 0; i < numKeys; i++) {
    trits = trits_from_trytes(trytes + i * IOTA_FRAGMENT_LENGTH, IOTA_FRAGMENT_LENGTH);
  ...
  digestTrytes = trytes_from_trits((trit_t* const) digest, 0, numKeys*IOTA_HASHLEN_TRIT);
  free((void*)digest);
